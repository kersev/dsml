---
title: Week 2 - Statistical Computing with R
...
- Download R from CRAN (Comprehensive R Archive Network)
    ...
    - You then download other packages that you may need – more on this later
...
- ***Implicit coercion*** occurs so that every element in the vector is of the same class (Regress to lowest common denominator)
...
- 3 dimensional vectors are referred to as 3D arrays (no longer a matrix)
...
- A list is a vector that can contain elements of different modes/classes/data types
...
![](lecimages/1image14.png)

```{r}
p + geom_boxplot(aes(colour = drv))
```

![](lecimages/1image15.png)
...
```{r}
for(i in 1:50) {
  if(i <= 10) {## Skip the first 10 iterations
    next
  }
  # Do some calculations
}
